'''
This is the debug version of the AR_game.py file where I could see what I implemented
to know what to change or what works or doesn'T work :)
...
def update(dt):
    ...
    if saved_matrix is not None:
        ...
        if game_over is None:
            ...
            for e in enemies[:]:
                ...
                if board_x != -100:
                    dist = np.sqrt((e["x"] - board_x)**2 + (e["y"] - board_y)**2)
                    if dist < (e["radius"]+10):
    ...
    if saved_matrix is None:
        ...
        # End Game Text
        if game_over == "WIN":
            cv2.putText(init_warped_frame, "Congratulations you won", (CAM_WIDTH//2 - 250, CAM_HEIGHT//2), cv2.FONT_HERSHEY_SIMPLEX, 1.2, (0, 255, 0), 3)
        elif game_over == "LOSE":
            cv2.putText(init_warped_frame, "Wamp waamp 5 enemies missed, you lose", (CAM_WIDTH//2 - 350, CAM_HEIGHT//2), cv2.FONT_HERSHEY_SIMPLEX, 1.2, (255, 0, 0), 3)
